class Drawflow {
  constructor(container, render = null, parent = null, data = null) {
    this.events = {};
  ...
  start() {
    ...
    this.container.addEventListener("change", this.updateNodeValue.bind(this));
    ...
    this.initDebounce(); // Inicializa o debounce
  ...
  addNode(
    ...
    for (var x = 0; x < inp; x++) {
      ...
      json_inputs["input_" + (x + 1)] = { dados: opcao, connections: [] };
      inputs.appendChild(input);
    }

    const json_outputs = {};
    let out = num_out.length;
    ...
    for (var x = 0; x < out; x++) {
      ...
      label.innerText = opcao.codigo; // Texto da label
      label.classList.add("badge");
      label.classList.add("badge-dark");
      label.style.position = "absolute"; // Posicionamento absoluto
      label.style.top = "-2px"; // Posiciona a label acima da bolinha
      label.style.right = "200%"; // Centraliza a label
      label.style.fontSize = "8px"; // Tamanho da fonte
      label.style.color = "white"; // Cor do texto
    
      // Adiciona a label dentro da bolinha
      output.appendChild(label);
    
      // Adiciona a bolinha ao container de outputs
      outputs.appendChild(output);
    
      // Armazena os dados no objeto json_outputs
      json_outputs["output_" + (x + 1)] = { dados: opcao, connections: [] };
    }
  ...
  addNodeImport(dataNode, precanvas) {
    ...
    for (var x = 0; x < Object.keys(dataNode.outputs).length; x++) {
      let dout = dataNode.outputs[Object.keys(dataNode.outputs)[x]];
      const output = document.createElement("div");
      output.classList.add("output");
      output.classList.add("output_" + (x + 1));
      ...
      // Cria a label (span)
      const label = document.createElement("span");
      label.innerText = dout.dados.codigo; // Texto da label
      label.classList.add("badge");
      label.classList.add("badge-dark");
      label.style.position = "absolute"; // Posicionamento absoluto
      label.style.top = "-2px"; // Posiciona a label acima da bolinha
  ...
  addConn(objeto, value, valorCompleto) {
    this.addNodeOutput(objeto.idNode, value, valorCompleto);
    // Aqui você pode adicionar a lógica para a função addConn
  }

  consultarObjeto(objeto, outputs) {
    console.log({ outputs: outputs });
  
    // Expressão regular para capturar o padrão desejado
    const regex =
      /\/\/\*\*\/\/fim:output:{{nomeElemento}}:{{idElemento}}:(.*?)\/\/\*\*\/\//g;
  
    // Itera sobre o objeto para encontrar correspondências
    for (let key in objeto) {
      if (objeto.hasOwnProperty(key)) {
        const valor = objeto[key];
        let match;
  
        // Reinicia a busca para cada valor
        regex.lastIndex = 0;
  
        // Verifica se há correspondências no valor atual
        while ((match = regex.exec(valor)) !== null) {
          const saida = match[1];
          this.addConn(objeto, saida, objeto[key]);
        }
      }
    }
  
    // Verifica se outputs existe e não está vazio
    if (outputs && Object.keys(outputs).length > 0) {
      let outs = Object.values(outputs);
      let count = 0;
      let keys = Object.keys(outputs);
      // Itera sobre os valores de outputs
    
        // Itera sobre os valores de outputs usando for
        for (let i = 0; i < outs.length; i++) {
          const e = outs[i];
        let keyAtual = keys[count];
        count++;
        console.log({ e: e });
  
        // Verifica se e.dados.personalizado é igual a 1
        if (e.dados && e.dados.personalizado === 1) {
          // Lógica específica para quando personalizado é 1
          const regexPersonalizado =
            new RegExp(`\/\/\\*\\*\/\/fim:output:{{nomeElemento}}:{{idElemento}}:${e.dados.codigo}(.*?)\/\/\\*\\*\/\/`, 'g');
  
          // Flag para indicar se o valor foi encontrado
          let encontrado = false;
  
          // Itera novamente sobre o objeto para aplicar a lógica personalizada
          for (let key in objeto) {
            if (objeto.hasOwnProperty(key)) {
              const valor = objeto[key];
              let match;
              ...
              // Reinicia a busca para cada valor
              regexPersonalizado.lastIndex = 0;
  
              // Verifica se há correspondências no valor atual
              while ((match = regexPersonalizado.exec(valor)) !== null) {
                const saidaPersonalizada = match[1];
                console.log(`Informação personalizada encontrada: ${saidaPersonalizada}`);
                encontrado = true; // Marca como encontrado
                // Lógica adicional para a informação personalizada
                // this.addConn(objeto, saidaPersonalizada, objeto[key]);
              }
            }
          }
  
          // Se o valor não foi encontrado em nenhuma chave do objeto
          if (!encontrado) {
            console.log({keyAtual:keyAtual,objeto:objeto.idNode});
            
           this.removeNodeOutput(objeto.idNode,keyAtual);
          }
        }
       }
    }
    ...
    for (var i = 0; i < attr.length; i++) {
      if (attr[i].nodeName.startsWith("df-")) {
        var keys = attr[i].nodeName.slice(3).split("-");
          ...
          var outputs = this.drawflow.drawflow[this.module].data[
            event.target
              .closest(".drawflow_content_node")
              .parentElement.id.slice(5)
          ].outputs;
        ...
        if (event.target.tagName.toLowerCase() === "textarea") {
          // Caso seja um textarea, usa o value
          target[keys[keys.length - 1]] = event.target.value;
        } else if (event.target.isContentEditable) {
          // Caso seja contentEditable, usa o innerText
          target[keys[keys.length - 1]] = event.target.innerText;
        } else {
          // Caso padrão (input, select, etc.), usa o value
          target[keys[keys.length - 1]] = event.target.value;
        }
        target["idNode"] = event.target
          .closest(".drawflow_content_node")
          .parentElement.id.slice(5);
        this.debouncedConsultarObjeto(target,outputs);
        this.dispatch(
          "nodeDataChanged",
          event.target
            .closest(".drawflow_content_node")
  ...
  // Método para inicializar o debounce
  initDebounce() {
    this.debouncedConsultarObjeto = debounce(
      this.consultarObjeto.bind(this),
      1000
    ); // 500ms de espera
  }
  updateNodeDataFromId(id, data) {
    ...
    console.log({datadata:data,id: id});
    console.log({todos:this.drawflow.drawflow[moduleName].data});
  ...
  updateNodeDataFromIdNotRemove(id, data) {
    var moduleName = this.getModuleFromNodeId(id);
    this.drawflow.drawflow[moduleName].data[id].data = {
      ...this.drawflow.drawflow[moduleName].data[id].data, // Objeto original
      ...data // Objeto com as atualizações
    };
    if (this.module === moduleName) {
      const content = this.container.querySelector("#node-" + id);
  ...
  /*
  addNodeOutput(id,valores = null) {
    console.log({ asasasasasas: id });
    ...
    ] = { dados:{nome: valores,codigo:valores,personalizado:1}, connections: [] };
  }
    */

  addNodeOutput(id, valores = null, valorCompleto = null) {

    var moduleName = this.getModuleFromNodeId(id);
    const infoNode = this.getNodeFromId(id);

    // Verifica se o valorCompleto contém o padrão esperado

    // Verifica se já existe um output com o mesmo dados.codigo
    const outputExistente = Object.values(infoNode.outputs).find((output) => {
     
      return output.dados && output.dados.codigo === valores;
    });
    const numOutputs = Object.keys(infoNode.outputs).length;

    if (outputExistente) {
      console.log(
        "Output com o mesmo dados.codigo e dados.personalizado == 1 já existe. Nada será feito."
      );
      return; // Caso 1: Já existe, não faz nada
    }

    if (this.module === moduleName) {
      // Caso 2: Não existe, adiciona o novo output
      const output = document.createElement("div");
      output.classList.add("output");
      output.classList.add("output_" + (numOutputs + 1));
      const parent = this.container.querySelector("#node-" + id + " .outputs");
    ...
    // Adiciona o novo output ao drawflow
    this.drawflow.drawflow[moduleName].data[id].outputs[
      "output_" + (numOutputs + 1)
    ] = {
      dados: { nome: valores, codigo: valores, personalizado: 1 },
      connections: [],
    };
  ...
  removeNodeId(id) {
    ...
    console.log({id:id});
    if(this.drawflow.drawflow[moduleName].data && this.drawflow.drawflow[moduleName].data[id.slice(5)]){
